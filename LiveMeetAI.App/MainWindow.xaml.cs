    ...
    public partial class MainWindow : Window
    {
        ...
        private void MainWindow_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    FileLogger.Debug("Hotkey pressed id=" + id);
                    // map the ids 1,2 to start system/mic capture
                    if (id == 1)
                    {
                        Dispatcher.Invoke(() => StartSystem_Click(null, null));
                    }
                    else if (id == 2)
                    {
                        Dispatcher.Invoke(() => StartMic_Click(null, null));
                    }
                    else if (id == 3)
                    {
                        Dispatcher.Invoke(() => Stop_Click(null, null));
                    }
                };
                ...
                Task.Run(async () =>
                {
                    ...
                    if (diff >= chunkDebounce)
                    {
                        ...
                        if (!string.IsNullOrEmpty(candidate) && candidate.Length >= minimumCharactersToSend)
                        {
                            ...
                            // Only clear buffer if actually sent (or we decide to drop it)
                            // For simplicity, we assume we try to send and clear prompt.
        ...
        private void StartSystem_Click(object sender, RoutedEventArgs? e)
        {
            try
            {
                if (isSystemRunning)
                {
                    ...
                    AiBox?.AppendText("Stopped system capture\n");
                    FileLogger.Info("Stopped system capture");
                    return;
                }
                ...
                {
                    FileLogger.Debug("Audio chunk ready (system) size=" + (args.WavBytes?.Length ?? 0));
                    if (stt != null)
                    {
                        try
                        {
                        ...
                        {
                            FileLogger.Error("STT error: " + ex.Message);
                        }
                    }
                };
                ...
                AiBox?.AppendText("Started system capture\n");
                FileLogger.Info("Started system capture");
            }
            catch (Exception ex)
            {
                FileLogger.Error("StartSystem_Click failed: " + ex.Message);
        ...
        private void StartMic_Click(object sender, RoutedEventArgs? e)
        {
            try
            {
                if (isMicRunning)
                {
                    audioService?.Stop();
                    isMicRunning = false;
                    AiBox?.AppendText("Stopped mic capture\n");
                    FileLogger.Info("Stopped mic capture");
                    return;
                }
                ...
                audioService.OnChunkReady += async (s, args) =>
                {
                    FileLogger.Debug("Audio chunk ready (mic) size=" + (args.WavBytes?.Length ?? 0));
                    if (stt != null)
                    {
                        try
                        {
                ...
                isMicRunning = true;
                AiBox?.AppendText("Started microphone capture\n");
                FileLogger.Info("Started microphone capture");
            }
            catch (Exception ex)
            {
                FileLogger.Error("StartMic_Click failed: " + ex.Message);
        ...
        private void Stop_Click(object sender, RoutedEventArgs? e)
        {
            try
            {
                ...
                FileLogger.Info("Capture stopped by user");
            }
            catch (Exception ex)
            {
                FileLogger.Error("Stop_Click failed: " + ex.Message);
